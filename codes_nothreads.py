...
long = len(urllist)
for i in range(long):
    link = 'http://music.163.com/song/media/outer/url?id={0}.mp3'.format(urllist[i])
    with open(r'C:\Users\qiuchunliu\Desktop\music'
              r'download\{0}.mp3'.format(songnamelist[i].replace('?', '')), 'wb') as song:
        # 就把文件放在桌面吧，使用时请替换掉 * 。
        song.write(requests.get(link).content)
        # 用requests.get() 方法来下载
        print('%s 下载完成' % songnamelist[i])
