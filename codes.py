...
# os.mkdir(r'C:\Users\qiuchunliu\Desktop\musicdownload')
# 在桌面创建一个文件夹放文件
long = len(urllist)
item = []
for i in range(long):
    item.append([urllist[i], songnamelist[i]])


def main(ite):
    link = 'http://music.163.com/song/media/outer/url?id={0}.mp3'.format(ite[0])
    with open(r'C:\Users\qiuchunliu\Desktop\musicdownload\{0}.mp3'.format(ite[1].replace('?', '')), 'wb') as song:
        # 就把文件放在桌面吧，使用时请替换掉 * 。
        song.write(requests.get(link).content)
        # 用requests.get() 方法来下载
        print('%s 下载完成' % ite[1])
    # print('\r正在下载第 {0} 首歌。共有{1}'.format(count, long), end='')
    # \r 可以实现原地显示，而不是循环一次输出一行


for x in item:
    print('下载中，请稍后。。。')
    th = threading.Thread(target=main, args=[x])
    # 第一个参数是目标函数，就是将要执行的函数
    # 第二个参数是将要执行的函数需要的参数，可以是多个，也可以是一个
    th.start()
